...
DROP PROCEDURE ConvertirMontoNumerico_a_Texto
GO

CREATE PROCEDURE ConvertirMontoNumerico_a_Texto
	@monto NUMERIC(14,2),
	@moneda CHAR(10),
...
SET NOCOUNT ON
	...
	IF @decimal > 0
	 SELECT @decimales = '  '  +     REPLICATE ('0' ,2 - LEN(RTRIM(LTRIM(CAST (@decimal AS char(2)))))) +
		 LTRIM(RTRIM(CAST (@decimal AS char(2))))   +     '/100'
	ELSE
	 SELECT @decimales = ' 00/100' 
	

	--SELECT @letrAS  = '** '    +    RTRIM(SUBSTRING(@letrAS,1,255)) +    RTRIM(@decimales)  +    ' **'
	SELECT @letrAS  = UPPER(LTRIM(RTRIM( RTRIM(SUBSTRING(@letrAS,1,255)) +    RTRIM(@decimales)))) + ' '+ UPPER(@moneda)
	--SELECT 'letrAS' = UPPER(@letrAS)
END
GO

--DEClARE @letrAS varchar(250)
--exec ConvertirMontoNumerico_a_Texto '421.0443','Bolivianos',@letrAS output
--SELECT @letrAS
