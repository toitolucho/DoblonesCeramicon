...
{
    public partial class FIngresarCodigoProductoEspecifico : Form
    {
        ...
        private bool generadosPorSistema = false;
        #endregion
        ...
        public FIngresarCodigoProductoEspecifico(int NumeroAgencia)
        {
            InitializeComponent();
        ...
        public void formatearEstiloProductosEspecificos()
        {
            if (DTProductosEspecificosTemporal.Columns.Count == 0)
            {
                ...
                for (int i = 0; i < DTDatosTransaccion.Rows.Count; i++ )
                {
                    ...
                    {
                        CodigoProductoAnterior = DTDatosTransaccion.Rows[i]["Código Producto"].ToString().Trim();
                        break;
                    }
            ...
            PrimaryKeyColumnsDTDatosTransaccion[0] = DTDatosTransaccion.Columns["Código Producto"];
        ...
        private void btnCancelarTodo_Click(object sender, EventArgs e)
        {
            ...
            else
            {
                if (cBoxTipoAgregado.Visible)
                {
                    object cantidadTotalProductos = DTDatosTransaccion.Compute("sum(Cantidad)", "VendidoComoAgregado = true");
                ...
                {
                    object cantidadTotalProductos = DTDatosTransaccion.Compute("sum(Cantidad)", "EsProductoEspecifico = 1");
                    int TotalProductos = Int32.Parse(cantidadTotalProductos.ToString());
        ...
        private void btnConfirmar_Click(object sender, EventArgs e)
        {
            ...
            {                
                ...
                foreach (DataRow filaDatos in DTDatosTransaccion.Rows)
                {
                    ...
                    {
                        cantidad = Int32.Parse(filaDatos["Cantidad"].ToString());
                        CodigoProducto = filaDatos["Código Producto"].ToString().Trim();
                        ...
                        if (Int32.TryParse(cantidadPorProducto.ToString(), out CantidadInsertadaActualmentePorProducto))
                        {
                            ...
                            {
                                string NombreProducto = filaDatos["Nombre Producto"].ToString().Trim();
